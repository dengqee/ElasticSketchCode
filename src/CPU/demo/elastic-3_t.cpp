...
for(numNode=13;numNode<=23;numNode++){
	...
	string outdir="/home/dengqi/eclipse-workspace/ElasticSketchCode/data/multiswitch/elastic/"+to_string(numNode)+"/";
...
//	for(test=1;test<=1;test++)
	...
	//	ifstream ifs(nodecapfile.c_str());
		//读取测量节点的容量
		vector<int>cap;
//		string capfile=dir+"packets_subbalanced/rand"+to_string(test)+"/capacity.txt";
		string capfile=dir+"packets_subbalanced/"+to_string(test)+"/capacity.txt";//这是为了跑所有节点容量设置为一样，这里test是cm的宽度
...
/*********************************original*************************************************/
		...
		for(int node = 0; node < numNode; ++node)
		{
			...
			elastic = new ElasticSketch<BUCKET_NUM>(cmcounter_num);
			//测量
...
/******************************************** balanced ***************************************/
		...
		for(int node = 0; node < numNode; ++node)
		{
			...
			for(int i = 0; i < packet_cnt; ++i)
			{
				...
				int est=elastic->query((uint8_t*)traces_balanced[node][i].c_str());
...
/****************************************random**********************************************/
		...
		for(int node = 0; node < numNode; ++node)
		{
			...
			for(int i = 0; i < packet_cnt; ++i)
			{
				...
				int est=elastic->query((uint8_t*)traces_random[node][i].c_str());
...
/***************************************subbalanced*****************************************/
		...
		allReal.clear();
		allEST.clear();
//		tracedir=dir+"packets_subbalanced/rand"+to_string(test)+"/";
		...
		for(int node = 0; node < numNode; ++node)
		{
			...
			for(int i = 0; i < packet_cnt; ++i)
			{
				elastic->insert((uint8_t*)traces_subbalanced[node][i].c_str());
				int est=elastic->query((uint8_t*)traces_subbalanced[node][i].c_str());
