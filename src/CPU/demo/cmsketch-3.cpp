...
for(numNode=13;numNode<=23;numNode++){
	...
	string outdir="/home/dengqi/eclipse-workspace/ElasticSketchCode/data/multiswitch/cm/"+to_string(numNode)+"/";
...
/*********************************original*************************************************/
		...
		for(int node = 0; node < numNode; ++node)
		{
			...
			cm = new CMSketch<4, 4>(cmcounter_num*4);
			//测量
			for(int i = 0; i < packet_cnt; ++i)
			{
				cm->insert((uint8_t*)traces_origin[node][i].c_str());
				int est=cm->query((uint8_t*)traces_origin[node][i].c_str());
...
/******************************************** balanced ***************************************/
		...
		for(int node = 0; node < numNode; ++node)
		{
			...
			for(int i = 0; i < packet_cnt; ++i)
			{
				cm->insert((uint8_t*)traces_balanced[node][i].c_str());
				int est=cm->query((uint8_t*)traces_balanced[node][i].c_str());
...
/****************************************random**********************************************/
		...
		for(int node = 0; node < numNode; ++node)
		{
			...
			for(int i = 0; i < packet_cnt; ++i)
			{
				cm->insert((uint8_t*)traces_random[node][i].c_str());
				int est=cm->query((uint8_t*)traces_random[node][i].c_str());
...
/***************************************subbalanced*****************************************/
		...
		for(int node = 0; node < numNode; ++node)
		{
			...
			for(int i = 0; i < packet_cnt; ++i)
			{
				cm->insert((uint8_t*)traces_subbalanced[node][i].c_str());
				int est=cm->query((uint8_t*)traces_subbalanced[node][i].c_str());
